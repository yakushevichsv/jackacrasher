    ...
    [relevantImages enumerateObjectsUsingBlock:^(NSDictionary *obj, NSUInteger idx, BOOL *stop) {
        ...
        [subimages enumerateObjectsUsingBlock:^(NSDictionary *obj, NSUInteger idx, BOOL *stop) {
            ...
            if(scale == 2.0) {
                ...
                filenameWithoutExtension = [filenameWithoutExtension stringByReplacingOccurrencesOfString:@"@3x" withString:@""];
            }
...
+ (UIImage *)spriteWithContentsOfAtlas:(NSString *)filename name:(NSString *)name {
    NSDictionary *atlas = [UIImage atlas:filename];
    
    UIImage *sprite = [atlas objectForKey:name];
